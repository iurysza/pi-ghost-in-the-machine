...
- The watcher detects only. `ghost-state.sh` remains authoritative for sidebar gating, pane memory, active shader state, and collapse races.
...
The only long-lived helper is the per-socket Node watcher. It uses no steady-state child processes; Bash runs only on sidebar transitions. Small files and a signal still form the Ghostty handoff. See [[ai-artifacts/docs/semantic-map|semantic map]] for ownership and [[ai-artifacts/docs/operations-and-verification|operations]] for failure isolation.
